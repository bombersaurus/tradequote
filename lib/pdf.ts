...
export function exportQuotePdf(quote: Quote, paidUnlocked: boolean): void {
  ...
  doc.text("Job address", 306, y);
  y += 16;
  doc.setFont("helvetica", "normal");
  doc.text([quote.customer.name, quote.customer.email, quote.customer.phone].filter(Boolean), 48, y);
  doc.text(doc.splitTextToSize(quote.jobAddress, 230), 306, y);
  ...
  y += 18;
  totalRow(doc, `Deposit (${quote.depositPercent}%)`, totals.deposit, y, pageWidth);
  ...
  if (!paidUnlocked || quote.includeBranding) {
    doc.setFontSize(9);
